  ...
  return (
    <section>
      {sections.map((Section) => (
        <section
          key={Section.id}
          className="border-t w-full border-gray-300 flex flex-col lg:flex-row items-center px-[6%] lg:px-30 py-10 md:py-12"
        >
          <div className="flex flex-col gap-4 items-start">
            <p className="mb-20 font-poppins text-[14px] text-black font-light">
              {Section.topic}
            </p>
            <p className="font-poppins text-brand2 text-[20px] font-normal">
              {Section.tag}
            </p>
            <h2 className="font-Montserrat font-medium text-[38px] md:text-[48px] md:leading-15">
              {Section.title}
            </h2>
            <p className="font-poppins font-normal text-[16px] text-gray-700 mt-4">
              {Section.text}
            </p>
            <a
              href=""
              className="text-white bg-blues w-[55%] flex items-center justify-center p-2 cursor-pointer border border-blues mt-10 font-poppins text-[16px] font-normal hover:bg-gray-600 duration-200 transition-colors"
            >
              {Section.button}
            </a>
          </div>
